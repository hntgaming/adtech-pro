 ...
 */
function HTG_metaboxes_call() {
    new HTG_Metaboxes();
}
 
if ( is_admin() ) {
    add_action( 'load-post.php',     'HTG_metaboxes_call' );
    add_action( 'load-post-new.php', 'HTG_metaboxes_call' );
}
...
class HTG_Metaboxes {
    ...
    public function save( $post_id ) {
        ...
        if ( ! isset( $_POST['HTG_layout_metabox_nonce'] ) ) {
            return $post_id;
        }
 
        $nonce = $_POST['HTG_layout_metabox_nonce'];
 
        // Verify that the nonce is valid.
        if ( ! wp_verify_nonce( $nonce, 'HTG_layout_metabox' ) ) {
            return $post_id;
        }
        ...
        $selected_layout = sanitize_text_field( $_POST['HTG_layout'] );
 
        // Update the meta field.
        update_post_meta( $post_id, '_HTG_layout_meta', $selected_layout );
    }
    ...
    public function render_meta_box_content( $post ) {
        ...
        wp_nonce_field( 'HTG_layout_metabox', 'HTG_layout_metabox_nonce' );
 
        // Use get_post_meta to retrieve an existing value from the database.
        $selected_layout = get_post_meta( $post->ID, '_HTG_layout_meta', true );
